...
runOption (Explore f stmode cutoffs) = 
    if stmode == 0
    then explore f False
    else explore f True
runOption Test = test
...
explore :: FilePath -> Bool -> IO ()
explore f mode = do
  prog <- extract f
  let (prog', fflow, flow, thcount) = frontEnd prog
      (size,nconf, unfst) = runST (convert prog' fflow flow thcount >>= uncurry (Exp.stateless mode) >>= unfToDot)
