...
explore f mode cutoffs = do
  ...
  prog <- extract f
  let (prog', fflow, flow, thcount) = frontEnd prog
      (size,nconf, ncutoff, sumsize) = runST (convert prog' fflow flow thcount >>= uncurry (Exp.unfolder mode cutoffs) >>= \s -> return (cntr s, maxConf s, cutoffCntr s, cumsize s)) --unfToDot)
...
debug f mode cutoffs = do 
    prog <- extract f
    let (prog', fflow, flow, thcount) = frontEnd prog
        pes = runST (convert prog' fflow flow thcount >>= uncurry (Exp.unfolder mode cutoffs) >>=  \s -> showEvents $ evts s)
    putStrLn pes
