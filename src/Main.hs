...
import qualified Exploration.UNF.Unfolderless as Exp
...
data Option = Frontend {input :: FilePath}
            ...
            | Explore {input :: FilePath, stateless :: Int, cutoff :: Int}
            | Test 
...
exploreMode = Explore {input = def &= args
                      ,stateless = def &= help "stateless mode (0=False [default], 1=True)"
...
runOption (Explore f stmode cutoffs) =  explore f (toBool stmode) (toBool cutoffs)
runOption Test = test
...
explore f mode cutoffs = do
  prog <- extract f
  let (prog', fflow, flow, thcount) = frontEnd prog
      (size,nconf, ncutoff, sumsize) = runST (convert prog' fflow flow thcount >>= uncurry (Exp.stateless mode cutoffs) >>= \s -> return (cntr s, maxConf s, cutoffCntr s, cumsize s)) --unfToDot)
