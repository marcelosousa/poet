...
data Option = Frontend {input :: FilePath}
            ...
            | Explore {input :: FilePath, statmode :: Int}
            | Test 
...
exploreMode = Explore {input = def &= args
                      ,statmode = def &= help "stateless mode (0=False [default], 1=True)"} &= help _helpExplore
...
explore f mode = do
  prog <- extract f
  let (prog', fflow, flow, thcount) = frontEnd prog
      (size,nconf, unfst) = runST (convert prog' fflow flow thcount >>= uncurry (stateless mode) >>= unfToDot)
