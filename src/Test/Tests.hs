...
test8  = 
  let r1 = runST (sys1 >>= \sys -> stateless True False sys ind11 >>= return . show . maxConf)
      r2 = runST (sys1 >>= \sys -> stateless False False sys ind11 >>= return . show . maxConf)
  in TestCase (assertEqual "WxWx" r1 r2)

test9 = 
  let r1 = runST (sys1 >>= \sys -> stateless True False sys ind12 >>= return . show . maxConf)
      r2 = runST (sys1 >>= \sys -> stateless False False sys ind12 >>= return . show . maxConf)
  in TestCase (assertEqual "WxWy" r1 r2)

test10 = 
  let r1 = runST (sys2 >>= \sys -> stateless True False sys ind2 >>= return . show . maxConf)
      r2 = runST (sys2 >>= \sys -> stateless False False sys ind2 >>= return . show . maxConf)
  in TestCase (assertEqual "WxRxRx" r1 r2)

test11 = 
  let r1 = runST (sys3 >>= \sys -> stateless True False sys ind3 >>= return . show . maxConf)
      r2 = runST (sys3 >>= \sys -> stateless False False sys ind3 >>= return . show . maxConf)
  in TestCase (assertEqual "paper" r1 r2)

test12 = 
  let r1 = runST (sys4 >>= \sys -> stateless True False sys ind4 >>= return . show . maxConf)
      r2 = runST (sys4 >>= \sys -> stateless False False sys ind4 >>= return . show . maxConf)
  in TestCase (assertEqual "ex4" r1 r2)

test13 = 
  let r1 = runST (sys5 >>= \sys -> stateless True False sys ind5 >>= return . show . maxConf)
      r2 = runST (sys5 >>= \sys -> stateless False False sys ind5 >>= return . show . maxConf)
  in TestCase (assertEqual "cesar" r1 r2)

test14 = 
  let r1 = runST (sys6 >>= \sys -> stateless True False sys ind6 >>= return . show . maxConf)
      r2 = runST (sys6 >>= \sys -> stateless False False sys ind6 >>= return . show . maxConf)
...
test16 = 
  let r1 = runST (sys7 >>= \sys -> stateless True False sys ind7 >>= return . show . maxConf)
      r2 = runST (sys7 >>= \sys -> stateless False False sys ind7 >>= return . show . maxConf)
...
test18 = 
  let r1 = runST (sys8 >>= \sys -> stateless True False sys ind8 >>= return . show . maxConf)
      r2 = runST (sys8 >>= \sys -> stateless False False sys ind8 >>= return . show . maxConf)
