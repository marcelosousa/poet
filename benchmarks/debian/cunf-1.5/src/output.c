...
#define P(args...) fprintf2 (f, ##args)

	f = fopen (filename, "w");
	if (f == 0) {
		gl_err ("file '%s': error %s");
		return;
	}
	...
	P ("}\n");
	i = fclose (f);
	if (i == EOF) gl_err ("file '%s': error %s");
}
...
static void _write_int (const char * path, FILE * f, unsigned long int i)
{
	...
	ret = fwrite (&i, 4, 1, f);
	if (ret != 1) gl_err ("file '%s': error %s");
}
...
static void _write_str (const char * path, FILE * f, const char * str)
{
	...
	i = strlen (str) + 1;
	if (fwrite (str, i, 1, f) != 1) gl_err ("file '%s': error %s");
}
