		   ...
		   t_blockinfo *blocks, t_blockdest *dest)
{
	...
	if (!(infile = fopen(filename, "r")))
		gl_err ("'%s': cannot open for reading");
	...
	if (!*types) gl_err ("unexpected format identifier '%s'");
	...
	while (!feof(infile))
	{
		...
		for (; blocks->name && strcmp(blocks->name,sbuf); blocks++)
		    if (!blocks->optional)
			gl_err ("keyword '%s' expected");

		if (!blocks->name) gl_err ("unknown keyword '%s'");
			...
			switch(tbl[ch].type)
			{
				...
				if (tbl[ch].ptr)
					*(char**)(tbl[ch].ptr) = gl_strdup(sbuf);
					...
					sprintf2(rtmp,"\"%s\"",sbuf);
				else
					sprintf2(rtmp,"%c\"%s\"",ch,sbuf);
				break;
			    case FT_NUMBER:
				if (tbl[ch].ptr)
					*(int*)(tbl[ch].ptr) = num;
				...
				if (tbl[ch].ptr)
				{
				    *((t_coords**)(tbl[ch].ptr)) =
					...
					sprintf2(rtmp,"%d@%d ",num,num2);
				else
					sprintf2(rtmp,"%c%d@%d",ch,num,num2);
				break;
			    case FT_FLAG:
				if (tbl[ch].ptr)
					(*(int*)(tbl[ch].ptr))++;
...
static int _insert_place()
{
	...
	if (rd_ident > AnzPlNamen)
		...
		gl_err ("place identifier %d used twice");
	...
	if (rd_marked > 1) gl_err ("place %s has more than one token");
...
static int _insert_trans()
{
	...
	if (rd_ident > AnzTrNamen)
		...
		gl_err ("transition identifier %d used twice");
